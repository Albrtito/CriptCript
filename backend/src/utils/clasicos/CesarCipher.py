"""
Métodos encargados de cifrar y descifrar un mensaje en cifrado cesar (Cifrado
ROT)
"""
def caesar_cipher(message: str) -> dict:
    ...
    if len(cipheredMessage) != len(messageToCipher):
        raise ValueError(
            "Error: Something went wrong during ciphering, message lengths do not match."
        )
...
def caesar_decipher(ciphered_message: str) -> str:
    ...
    if len(decipheredMessage) != len(messageToDecipher):
        raise ValueError(
            "Error: Something went wrong during deciphering, message lengths do not match."
        )
